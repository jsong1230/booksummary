...
- [ ] **네이버 검색광고 API 키 발급** (사용자 작업) — Trends 는 두 한계가 실측됐다:
      ① 앵커보다 작은 키워드가 0으로 반올림(20편 중 9편이 0, 그 안에 조회 11~297회 혼재)
      ② **20~30건이면 429로 IP가 몇 시간 차단** — 후보 수십 건 계량이 불가능하다
      → 절대 검색수를 주는 네이버가 필요. https://searchad.naver.com → 도구 > API 사용 관리 (무료)
      `.env` 에 NAVER_AD_API_KEY / NAVER_AD_SECRET_KEY / NAVER_AD_CUSTOMER_ID
- [ ] **새 기준 9편 수요 측정 마무리** — 429로 7편(만세전·우상의 눈물·수난이대·유자소전·
      만무방·기억 속의 들꽃·레디메이드 인생)이 미측정. 차단 해제 후 또는 네이버 키로
- [ ] **수요×공급 결합 판정 규칙 확정** — 지금은 두 지표가 따로 논다.
      표본이 더 쌓이면(12월 판독 시점) 게이트 값을 정한다
