...
class TTSEngine:
    ...
    def generate_speech(
        ...
        self.logger.info("🎤 TTS 음성 생성 중...")
        self.logger.info(f"   음성: {voice}")
        self.logger.info(f"   모델: {model}")
        ...
        try:
            ...
            if len(text) <= MAX_CHARS:
                ...
                self.logger.warning(f"   ⚠️ 텍스트가 {len(text)}자로 너무 깁니다. 여러 청크로 나눠서 생성합니다.")
                ...
                self.logger.info(f"   📦 {len(chunks)}개의 청크로 분할됨")
                
                # 각 청크를 TTS로 변환
                audio_files = []
                for i, chunk in enumerate(chunks):
                    self.logger.info(f"   [{i+1}/{len(chunks)}] 청크 생성 중... ({len(chunk)}자)")
                ...
                # 오디오 파일들을 연결
                self.logger.info(f"   🔗 {len(audio_files)}개의 오디오 파일 연결 중...")
                try:
            ...
            self.logger.error(f"❌ TTS 생성 오류: {e}")
            raise
    ...
    def generate_from_file(
        ...
        self.logger.info("✅ Summary 파일 마크다운 정리 완료 (TTS 최적화)")
...
def main():
    ...
    if not args.text and not args.text_file:
        logger.error("❌ --text 또는 --text-file 중 하나를 지정해야 합니다.")
        return 1
